...
from .TimeDistributedLayer import TimeDistributedConv2d, TimeDistributedMaxPool, \
...
class SA_Unet_Torch(nn.Module):
    ...
    def __init__(self, image_size, device, n_class=3, block_size=7, keep_prob=0.9, start_neurons=64, kernel_size=3):
        ...
        self.encode_block1 = nn.Sequential(
            ...
            nn.ELU(),
            TimeDistributedConv2d(start_neurons * 1, self.start_neurons * 1, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU()
        )

        self.encode_block2 = nn.Sequential(
            TimeDistributedMaxPool(2, stride=2),
            TimeDistributedConv2d(self.start_neurons * 1, self.start_neurons * 2, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
        ...
        self.encode_block3 = nn.Sequential(
            TimeDistributedMaxPool(2, stride=2),
            TimeDistributedConv2d(self.start_neurons * 2, self.start_neurons * 4, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
        ...
        self.biCLSTM1 = BiConvLSTM(input_size=(image_size // (8), image_size // (8)), input_dim=512,
                                   hidden_dim=512, kernel_size=(self.kernel_size, self.kernel_size), num_layers=3, device=self.device)

        self.spatial_attention = TimeDistributedSpatialAttention()

        self.structured_dropout_convolutional_block_pre = nn.Sequential(
            TimeDistributedMaxPool(2, stride=2),
            TimeDistributedConv2d(self.start_neurons * 4, self.start_neurons * 8, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU()
        )
        ...
        self.unconv_block3 = nn.Sequential(
            TimeDistributedConv2d(self.start_neurons * 8, self.start_neurons * 4, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
            TimeDistributedConv2d(self.start_neurons * 4, self.start_neurons * 4, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
        ...
        self.unconv_block2 = nn.Sequential(
            TimeDistributedConv2d(self.start_neurons * 4, self.start_neurons * 2, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
            TimeDistributedConv2d(self.start_neurons * 2, self.start_neurons * 2, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
        ...
        self.biCLSTM2 = BiConvLSTM(input_size=(image_size, image_size), input_dim=64, hidden_dim=64, kernel_size=(self.kernel_size, self.kernel_size),
                                   num_layers=3, device=self.device)

        self.unconv_block1 = nn.Sequential(
            TimeDistributedConv2d(self.start_neurons * 2, self.start_neurons * 1, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU(),
            TimeDistributedConv2d(self.start_neurons * 1, self.start_neurons * 1, kernel_size=self.kernel_size, padding="same", dropout=True),
            nn.ELU()
        )
#######
        self.onebyoneConv = nn.Sequential(
            nn.Conv2d(self.start_neurons * 1, n_class, kernel_size=1, stride=1, padding="same",bias=False),
            nn.Sigmoid()
        )

        self.Attention1 = Attention_block(F_g=self.start_neurons * 1, F_l=self.start_neurons * 1, F_int=self.start_neurons * 1)
        self.Attention2 = Attention_block(F_g=self.start_neurons * 2, F_l=self.start_neurons * 2, F_int=self.start_neurons * 2)
        self.Attention3 = Attention_block(F_g=self.start_neurons * 4, F_l=self.start_neurons * 4, F_int=self.start_neurons * 4)
