...
from dataset import PatientList, PatientList_lb3, get_datasets
from trainer import OpthalCT_trainer, OpthalCT_seqTrainer
...
if __name__ == "__main__":
    ...
    base_dir = os.getcwd()
    attr={}
    attr['exp_name'] = str(opt.exp_name) + f'_tg{opt.target_lb}'+f'_kernel{opt.kernel_size}'
    attr['exp_folder'] = exp_folder_name

    out_dir, write_dir = get_result_dirs_ready(base_dir, attr)
    if opt.target_lb == 3:
        patient_list = PatientList_lb3(shuffle=True, total_fold=opt.total_fold_num)
    else:
    ...
    for fold in opt.fold_num:
        ...
        algo = OpthalCT_seqTrainer(epochs=opt.epoch,
                                    gpu=opt.device,
                                    ...
                                    writer=writer,
                                    target_label=opt.target_lb,
                                    test_prob=opt.test_plot_prob,
