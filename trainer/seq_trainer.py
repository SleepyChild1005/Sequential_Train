...
class sequentialSegTrainer(object):
    def __init__(self, epochs, gpu, batch_size, image_size, learning_rate, output_dir, model_type,
                 pretrained_model_dir, train_dataloader, test_dataloader, writer, target_label, test_prob, kernel_size=3):
        model_folder = 'model'
    ...
    def train(self):
        # for label2
        sqNet = model_getter(self.device, self.image_size, self.model_type, self.pretrained_model_dir, kernel_size=self.kernel_size)


        # for label1
        globe_model = model_getter(self.device, self.image_size, 'sensor_3d', self.pretrained_model_dir)
        globe_model.load_state_dict(torch.load(f'Data/PT_labels/Sensor3d/Label1/sqNet_best_label1.pth'))
        globe_model.eval()
        globe_model.requires_grad_(False)
        ...
        for epoch in range(self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                pred_globe = globe_model(seq_vols)
                pred_globe = torch.unsqueeze(pred_globe, dim=1)

                lb_concated_input = torch.cat([pred_globe, seq_vols], dim=1)

                sqNet.train()
                sqNet.requires_grad_(True)


                pred_masks = sqNet(lb_concated_input)


                mask_lb = get_label_mask(masks, self.target_label).to(self.device)
                predmask_lb = get_label_mask(pred_masks, 1).to(self.device)
                    ...
                    if total_step % result_print_step == 0:
                        with torch.no_grad():
                            ...
                            for train_result_idx, (gt, m, pred_m) in enumerate(zip(seq_vols, masks, pred_masks)):
                                ...
                                b_gtmask_lb = color_rgb_filter('gt', self.target_label)
                                b_predmask_lb = color_rgb_filter('pred', self.target_label)

                                # multiply rgb filter
                                gt = cv2.cvtColor(gt[:, :, 1] * 255, cv2.COLOR_GRAY2RGB)
                                b_gtmask_lb = mul_rgb(b_gtmask_lb, m[self.target_label-1, :, :])
            ...
            with torch.no_grad():
                ...
                for test_idx, data in enumerate(self.test_dataloader):
                    ...
                    test_pred_globe = globe_model(test_seq_vols)
                    test_pred_globe = torch.unsqueeze(test_pred_globe, dim=1)

                    test_lb_concated_input = torch.cat([test_pred_globe, test_seq_vols], dim=1)
                    test_pred_masks = sqNet(test_lb_concated_input)


                    test_mask_lb = get_label_mask(test_masks, self.target_label).to(self.device)
                    test_predmask_lb = get_label_mask(test_pred_masks, 1).to(self.device)
                    ...
                    if test_idx == 0 and epoch % test_img_export_interval == 0:
                        ...
                        for idx, (gt, m, pred_m, globe) in enumerate(zip(test_seq_vols, test_masks, test_pred_masks, test_pred_globe.cpu().numpy().squeeze())):
                            pred_m = np.where(pred_m > 0.5, 1, 0)
                            ## rgb color filters
                            b_gtmask_lb = color_rgb_filter('gt', self.target_label)
                            b_predmask_lb = color_rgb_filter('pred', self.target_label)

                            globe_pred_mask = color_rgb_filter('pred', 1)
                            gt = cv2.cvtColor(gt[:, :, 1] * 255, cv2.COLOR_GRAY2RGB)
                            # multiply rgb filter
                            b_gtmask_lb = mul_rgb(b_gtmask_lb, m[self.target_label-1, :, :])
                            b_predmask_lb = mul_rgb(b_predmask_lb, pred_m)

                            globe_pred_mask = mul_rgb(globe_pred_mask, globe)
                            # save result images
                            temp_result_gtmask = cv2.addWeighted(gt.astype(np.uint8), 0.7,
                                                                 b_gtmask_lb.astype(np.uint8), 0.5, 0)

                            temp_result_predmask = cv2.addWeighted(gt.astype(np.uint8), 0.7,
                                                                   globe_pred_mask.astype(np.uint8), 0.5, 0)
                            temp_result_predmask = cv2.addWeighted(temp_result_predmask, 1,
                                                                   b_predmask_lb.astype(np.uint8), 0.5, 0)
        ...
        # print(self.output_dir.split("\\"))
        out_list = self.output_dir.split("\\")
        ...
        f = open(self.model_dir + f'/best_worst_dice_score_{out_list[-3]}_{out_list[-2]}_{out_list[-1]}.txt', 'w')

        # summary
        f.write("Experiment_Name : " + out_list[-3] + "/" + out_list[-2] + "/" +
